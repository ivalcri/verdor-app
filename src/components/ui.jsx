...
export const Btn = ({ t, children, variant = 'primary', onClick, full, style, disabled }) => {
  const variants = {
...
export const Card = ({ t, children, style, pad = 18, onClick, raised = false }) => (
  <div
    onClick={onClick}
    style={{
...
export const Pill = ({ t, children, color, style }) => (
  <span style={{
...
export const Ring = ({ t, size = 180, stroke = 14, value = 0, max = 100, color, track, label, sub, children }) => {
...
export const TripleRing = ({ t, size = 180, p, c, f }) => {
  const data = [
...
export const MacroBar = ({ t, label, value, goal, color, icon }) => {
...
export const TabBar = ({ t, active, onChange }) => {
  const tabs = [
  ...
  return (
    <div style={{
      ...
      padding: '8px 10px env(safe-area-inset-bottom, 20px)',
      background: t.bg,
...
export const TopBar = ({ t, title, eyebrow, back, onBack, action }) => (
  <div style={{
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {title && (
        <h1 style={{
          ...
          fontSize: 32, lineHeight: 1.05, letterSpacing: '-0.02em', color: t.ink,
        }}>{title}</h1>
      )}
    </div>
    {action}
...
export const FoodPlaceholder = ({ t, label, height = 200, src }) => (
  <div style={{
    width: height, height, borderRadius: t.radiusSm, overflow: 'hidden', position: 'relative', flexShrink: 0,
    background: src
    ...
    {!src && (
      <div style={{
        ...
        fontFamily: 'ui-monospace, "SF Mono", monospace', fontSize: 9, color: t.muted,
